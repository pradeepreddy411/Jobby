...
class Jobs extends Component {
  ...
  changeEmployeeList = employee => {
    ...
    const inputNotInList = employeeType.filter(each => each === employee)
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          employeeType: [...prevState.employeeType, employee],
        }),
        this.getJobs,
      )
    } else {
      const filteredData = employeeType.filter(each => each !== employee)
